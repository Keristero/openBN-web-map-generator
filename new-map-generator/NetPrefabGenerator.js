...
function GenerateForRequirements({ ground_features, wall_features }) {
  ...
  while (placed_features.ground_features < ground_features) {
    ...
    let chosen_part =
      parts_with_ground_features[
        Math.floor(Math.random() * parts_with_ground_features.length)
      ];
    let new_placement_position = find_placement(
      chosen_part,
      placements,
      placed_features
    );
    ...
    console.log(
      `placed ground features`,
      placed_features.ground_features,
      "/",
      ground_features
    );
  }
  ...
  while (placed_features.wall_features < wall_features) {
    ...
    let chosen_part =
      parts_with_wall_features[
        Math.floor(Math.random() * parts_with_wall_features.length)
      ];
    let new_placement_position = find_placement(
      chosen_part,
      placements,
    ...
    console.log(
      `placed wall features`,
      placed_features.wall_features,
      "/",
      wall_features
    );
  }
...
function write_parts_to_prefab(prefab, placements) {
  ...
  let part_width = -Infinity;
  let part_length = -Infinity;
  let part_height = -Infinity;
  ...
  for (let placement of placements) {
    ...
    if (part_max_x > part_width) {
      part_width = part_max_x;
    }
    if (part_max_x > part_width) {
      part_length = part_max_y;
    }
    if (part_max_z > part_height) {
      part_height = part_max_z;
    }
...
function place_part(part, part_pos, placed_features, placements) {
  ...
  placed_features.arr_female_connectors =
    placed_features.arr_female_connectors.concat(part.female_connectors);
...
function find_placement(partA, placements, placed_features) {
  ...
  for (let male_connector of partA.male_connectors) {
    for (let female_connector of female_connectors) {
      ...
      if (
        !does_part_at_position_overlap_placed_part(partA, partA_pos, placements)
      ) {
        return partA_pos;
      }
    }
...
function does_part_at_position_overlap_placed_part(
  partA,
  partA_pos,
  placements
) {
  for (let placement of placements) {
    ...
    let z_overlap =
      partA_pos.z < partB_pos.z + partB.height &&
      partA_pos.z + partA.height > partB_pos.z;
    let y_overlap =
      partA_pos.y < partB_pos.y + partB.length &&
      partA_pos.y + partA.length > partB_pos.y;
    let x_overlap =
      partA_pos.x < partB_pos.z + partB.width &&
      partA_pos.x + partA.width > partB_pos.x;
